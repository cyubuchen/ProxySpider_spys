...
def thread_ckeck_proxy(proxy):
	...
	try:
		protocol = re.findall('(\w+?):\/\/', proxy)
		ip = re.findall('\/\/(.*)', proxy)
		if protocol[0] == 'http':
			rsp = requests.get(url_test, headers=headers, proxies={'http': proxy}, timeout=1, verify=False)
		else:
			rsp = requests.get(url_test, headers=headers, proxies={'https': proxy}, timeout=1, verify=False)
		if rsp.status_code == 200:
			file_path_checked = f'{os.getcwd()}/{CHECKED_PROXY}.txt'
			with open(file_path_checked,'a+') as f:
				f.write(proxy + '\n')
				print(f'CHECKING: {rsp.text.strip()} --> WORKING {proxy} Save to: {file_path_checked}\n')
		else:
			print(f'Not working: {proxy}\n')
	except (ProxyError, ConnectionError, ReadTimeout, Timeout, ReadTimeoutError):
...
def main():
	...
	for proxy in unchecked:
		threading.Thread(target=thread_ckeck_proxy, args=(proxy, )).start()
