    ...
    try:
        ...
        if rsp.status_code == 200:
            with open(file_path_checked, 'a+') as f:
                ...
                f.write('{0} | {1} | {2} | {3}\n{4}\n'.format(format(proxy, '<30'), format('{0:.2f}'.format(rsp.elapsed.total_seconds()), '<15'), format(ip_check[0][1], '<20'), ip_check[0][2], '-'*121))
        else:
            print('NOT WORKING: {0}\n'.format(proxy))
    except:
        print('BAD PROXY: {0}\n'.format(proxy))
...
def main():
    ...
    with open(file_path_checked,'w') as f:
        f.write('{0} | {1} |  {2} |  {3}\n{4}\n'.format(format('Proxy', '<30'), format('Response Time', '<15'), format('Region', '<19'), 'ISP', '-'*121))
    p = Pool(10)
